  ...
  async get(cacheId: string): AsyncResult<TweetWithContent | undefined, Error> {
    try {
      ...
      const tweet = new Tweet({
        createdAt: new Date(value.tweet.createdAt),
        id: value.tweet.id,
        videos: value.tweet.videos.map(props => new TweetMedia(props)),
        images: value.tweet.images.map(props => new TweetMedia(props)),
        user: user,
