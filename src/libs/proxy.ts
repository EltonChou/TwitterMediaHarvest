...
abstract class ExtensionStorageProxy<T extends LocalSchema | SyncSchema> implements IStorageProxy<T> {
  ...
  async removeItem<Key extends SchemaKey<T>>(keys: Key | Key[]): Promise<void> {
    await this.storageArea.remove(Array.isArray(keys) ? [...keys].map(v => String(v)) : String(keys))
  }
...
export class SimpleObjectStorageProxy<T extends LocalSchema | SyncSchema> implements IStorageProxy<T> {
  ...
  async removeItem<Key extends SchemaKey<T>>(keys: Key | Key[]): Promise<void> {
    Array.isArray(keys) ? keys.forEach(key => delete this.storage[String(key)]) : delete this.storage[String(keys)]
  }
}
